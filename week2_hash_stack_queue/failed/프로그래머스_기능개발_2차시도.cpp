...
vector<int> solution(vector<int> progresses, vector<int> speeds) {
    ...
    for (int i = 0; i < remainDayVector.size(); ++i) {
        ...
        if (dayQueue.empty()) {
            ...
            // 1. front 가 현재 남은 일 수보다 크다면
            if (front > currentRemainDay) {
                // 1-1. queue 에 현재 남은 일 수 push
                dayQueue.push(currentRemainDay);
            }
                // 2. front 가 현재 남은 일 수와 같을 때
            else if (front == currentRemainDay) {
                // 2-1. 앞의 모든 기능 (햔재 포함) 배포 가능해진다.
                dayQueue.push(currentRemainDay);
                ...
                // 3. front 가 현재 남은 일 수보다 작을 때
            else {
                // 3-1. 앞의 모든 기능 (현재 제외) 배포 가능해진다.
                answer.push_back(dayQueue.size());
                ...
                // 3-3. 비어있는 queue 에 현재 남은 일 수 push
                dayQueue.push(currentRemainDay);
            }
        }
    }
